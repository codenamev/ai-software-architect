...
#### Principles as a review gate (#38, C4)
`.architecture/principles.md` was copied into place by `setup-architect` and then read by nothing: no skill and no persona loaded it, so the project's constitution was a file the review never consulted. `architecture-review` now reads it in step 2 and hands the Core Principles section to every member inside the dispatch prompt (step 4). Each member returns a required "Principle conflicts" section naming any finding that violates a recorded principle and labeling any recommendation that would change one as PRINCIPLE CHANGE. Aggregation (new step 6.5) collects these into a **Principle Conflicts** section of `assets/review-template.md` under two rules: a finding tied to a principle is never bucketed below Important, and a proposed principle change never enters the prioritized recommendations, it is routed to `create-adr`. `create-adr` reads the same file and requires the ADR to name the principles it upholds and the ones it conflicts with; an ADR whose point is to change a principle edits `principles.md` in the same change. A missing principles file skips the gate with a note rather than blocking. Markdown only, no runtime change.
